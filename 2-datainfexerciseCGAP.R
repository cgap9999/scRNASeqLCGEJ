...
head(sce)
altExp(sce, 'ERCC')
result=match(rownames(altExp(sce)), rownames(cms))
table(is.na(result)) #checar si hay nas en todo el vector

head(rowData(altExp(sce)))
head(counts(sce))
head(counts(altExp(sce, 'ERCC'))) #obtener las counts de genes con ese ID
cms= cms[result,]
head(cms)
identical(rownames(altExp(sce, "ERCC")), rownames(cms))
toplot=cbind(counts(altExp(sce,'ERCC'))[,1], cms[,'concentration.in.Mix.1..attomoles.ul.'])
head(cms)
head(toplot)
pdf('counts1.pdf')
plot(toplot, main="Concentration in Mix 1 vs ERCC counts",xlab= "Concentration in Mix 1", ylab="Counts of a single cell")
dev.off()

for (i in 1:dim(counts(altExp(sce)))[2]){
  toplot=cbind(counts(altExp(sce,'ERCC'))[,i], cms[,'concentration.in.Mix.1..attomoles.ul.'])
  pdf(paste('counts', i,'.pdf', sep=""))
  plot(toplot, main=paste('Concentration in Mix 1 vs ERCC counts of cell ',i),xlab= "Concentration in Mix 1", ylab="Counts of a single cell")
  dev.off()
}
